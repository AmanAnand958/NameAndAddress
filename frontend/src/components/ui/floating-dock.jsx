...
const FloatingDockMobile = ({
  ...
  return <div className={cn("relative flex items-center gap-2 md:hidden", className)}>
      ...
      <AnimatePresence>
        {open && <motion.div layoutId="nav" className="flex flex-row gap-2">
...
const FloatingDockDesktop = ({
  ...
  return <motion.div onMouseMove={e => mouseX.set(e.pageX)} onMouseLeave={() => mouseX.set(Infinity)} className={cn("mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-gray-50 px-4 pb-3 md:flex dark:bg-neutral-900", className)}>
...
function IconContainer({
  ...
  const content = (
    <motion.div ref={ref} style={{
      width,
      height
    }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="relative flex aspect-square items-center justify-center rounded-full bg-gray-200 dark:bg-neutral-800">
        <AnimatePresence>
